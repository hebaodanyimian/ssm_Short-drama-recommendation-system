		...
		<div class="list-preview">
			...
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				<el-form-item class="list-item">
					<div class="lable">短剧标题：</div>
					<el-input v-model="formSearch.duanjubiaoti" placeholder="短剧标题" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-button class="list-search-btn" v-if=" true " type="primary" @click="getList(1, curFenlei)">
					<i class="el-icon-search"></i>
					查询
				</el-button>
				<el-button class="list-add-btn" v-if="btnAuth('duanjuxinxi','新增')" type="primary" @click="add('/index/duanjuxinxiAdd')">
			...
			<div class="list">
				...
				<div class="list1 index-pv1">
					<div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
						<img class="image" @click.stop="imgPreView(item.duanjutupian)" v-if="item.duanjutupian && item.duanjutupian.substr(0,4)=='http'&& item.duanjutupian.split(',w').length>1" :src="item.duanjutupian" />
						<img class="image" @click.stop="imgPreView(item.duanjutupian.split(',')[0])" v-else-if="item.duanjutupian && item.duanjutupian.substr(0,4)=='http'" :src="item.duanjutupian.split(',')[0]" />
						<img class="image" @click.stop="imgPreView(baseUrl + (item.duanjutupian?item.duanjutupian.split(',')[0]:''))" v-else :src="baseUrl + (item.duanjutupian?item.duanjutupian.split(',')[0]:'')" />
						<div class="name">{{item.duanjubiaoti}}</div>
						<div class="name">{{item.duanjufenlei}}</div>
						<div class="time_item">
...
<script>
	export default {
		...
		methods: {
			...
			async getFenlei() {
				await this.$http.get('option/duanjufenlei/duanjufenlei').then(res => {
					if (res.data.code == 0) {
			...
			getList(page, fenlei, ref = '') {
				...
				let searchWhere = {};
				if (this.formSearch.duanjubiaoti != '') searchWhere.duanjubiaoti = '%' + this.formSearch.duanjubiaoti + '%';
				if (this.curFenlei != '全部') searchWhere.duanjufenlei = this.curFenlei;
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`duanjuxinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/duanjuxinxiDetail', query: params});
			},
