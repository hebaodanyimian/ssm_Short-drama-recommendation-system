	...
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="短剧分类" prop="duanjufenlei">
				<el-input v-model="ruleForm.duanjufenlei" 
					placeholder="短剧分类" clearable :disabled=" false  ||ro.duanjufenlei"></el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`duanjufenlei/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`duanjufenlei/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
